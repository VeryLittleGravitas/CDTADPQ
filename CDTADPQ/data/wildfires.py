...
class FirePoint:
    def __init__(self, location, internal_id, usgs_id, name, contained, discovered, cause, acres):
...
def store_fire_point(db, fire_dict):
    ''' Add fire point to the db if the fire point does not already exist in the db
    '''
    db.execute('SELECT * FROM fire_points WHERE usgs_id = %s', (fire_dict['usgs_id'],))
    point_exists = db.rowcount
    if not point_exists:
        # Create point with location coordinates for insert
        fire_coordinates = fire_dict['location']['coordinates']
        ...
        db.execute('INSERT INTO fire_points (location, usgs_id, name, contained, discovered, cause, acres) VALUES (%s, %s, %s, %s, %s, %s, %s)',
         (coordinates, fire_dict['usgs_id'], fire_dict['name'], fire_dict['contained'], fire_dict['discovered'], fire_dict['cause'], fire_dict['acres']))
...
def convert_fire_point(feature):
    ...
    return dict(location=feature['geometry'], usgs_id=usgs_id, name=name, contained=contained,
                discovered=discovered, cause=cause, acres=acres)
...
def get_one_fire(db, usgs_id):
    ...
    return FirePoint(location, fire_row['id'], fire_row['usgs_id'], fire_row['name'],
...
def get_current_fires(db):
    ...
    for fire_row in fire_rows:
        ...
        fire_point = FirePoint(fire_location, fire_row['id'], fire_row['usgs_id'], fire_row['name'],
